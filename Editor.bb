...
If game=1 And screenAgenda<>5 And screenAgenda<>9
 blockData=1
 If charClause(gamChar,1)<2
  blockName=1 : blockAllegiance=1
  blockHair=1 : blockCostume=1 : blockEntrance=1
 EndIf
 If gamAgreement(1)>0 Then blockName=1
 If gamAgreement(2)>0 Then blockHair=1
 If gamAgreement(3)>0 Then blockCostume=1
 If gamAgreement(4)>0 Then blockEntrance=1
 If gamAgreement(5)>0 Then blockAttacks=1
 If gamAgreement(6)>0 Then blockMoves=1
 If gamAgreement(7)>0 Then blockTaunts=1
 If gamAgreement(8)>0 Then blockAllegiance=1  
 If gamAgreement(17)>0 Then blockWeapon=1
EndIf
...
While go=0
 ...
 For framer=1 To frames
	...
	If page=0 And keytim=0
	 ...
	 If KeyDown(57)
	  If foc=9 And blockData=0 And screenAgenda<>5
	   PlaySound sSwing : keytim=10
	   GenerateStoryRelationships(0)
	  EndIf
	  If foc=10 And blockData=0 And screenAgenda<>5
	   PlaySound sSwing : keytim=10
	   GenerateRealRelationships(0)
	  EndIf
	 EndIf
	EndIf
	...
	If page=2 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or (MouseDown(2) And MouseX()>rX#(400) And editExamine=0)
      ...
      If foc=12 And blockData=0 And screenAgenda<>5
       PlaySound sMenuBrowse : keytim=5 
       Repeat
	  ...
	  EndIf
      If foc=13 And blockData=0 And screenAgenda<>5
       PlaySound sMenuBrowse : keytim=5 
       Repeat
	 ...
	 If KeyDown(19) Or KeyDown(15) 
	  If foc=12 And charPartner(0)>0 Then charPartner(charPartner(0))=editChar : PlaySound sSwing : keytim=10
	  If foc=13 And charManager(0)>0 Then charManager(charManager(0))=editChar : PlaySound sSwing : keytim=10
	 EndIf
	 ;randomize
	 If KeyDown(57)
	  PlaySound sMenuBrowse : keytim=5
	 ...
	 If KeyDown(14)
      ...
      If foc=12 And blockData=0 Then charPartner(0)=0
      If foc=13 And blockData=0 Then charManager(0)=0
	 EndIf 
	EndIf
	...
	If page=3 And blockAttacks=0 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or (MouseDown(2) And MouseX()>rX#(400) And editExamine=0) 
	  ...
	  For count=1 To 5
	   If foc=count
	    Repeat
	     charAttack(0,count)=charAttack(0,count)-1
	     If charAttack(0,count)<1 Then charAttack(0,count)=attackList(count)
        Until attackLearned(count,charAttack(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=2 To 5
	   If foc=count+4
	    Repeat
	     charCrush(0,count)=charCrush(0,count)-1
	     If charCrush(0,count)<1 Then charCrush(0,count)=crushList(count)
        Until crushLearned(count,charCrush(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf
	 ;search right
	 If KeyDown(205) Or JoyXDir()=1 Or (MouseDown(1) And MouseX()>rX#(400) And editExamine=0)
	  ...
	  For count=1 To 5
	   If foc=count
	    Repeat
	     charAttack(0,count)=charAttack(0,count)+1
	     If charAttack(0,count)>attackList(count) Then charAttack(0,count)=1
        Until attackLearned(count,charAttack(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=2 To 5
	   If foc=count+4
	    Repeat
	     charCrush(0,count)=charCrush(0,count)+1
	     If charCrush(0,count)>crushList(count) Then charCrush(0,count)=1
        Until crushLearned(count,charCrush(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf
	 ...
	 If KeyDown(57)
	  ...
	  For count=1 To 5
	   If foc=count
	    Repeat
	     charAttack(0,count)=Rnd(1,attackList(count))
        Until attackLearned(count,charAttack(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=2 To 5
	   If foc=count+4 
	    Repeat
	     charCrush(0,count)=Rnd(1,crushList(count))
	    Until crushLearned(count,charCrush(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf 
	 ...
	 If KeyDown(14)
	  ...
	  For count=1 To 5
	   If foc=count 
	    If attackLearned(count,1)>0 Or game=0 Or screenAgenda=5 Then charAttack(0,count)=1
	   EndIf
	  Next
	  For count=2 To 5
	   If foc=count+4
	    If crushLearned(count,1)>0 Or game=0 Or screenAgenda=5 Then charCrush(0,count)=1
	   EndIf
	  Next
	 EndIf
	EndIf
	...
	If page=4 And blockMoves=0 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or (MouseDown(2) And MouseX()>rX#(400) And editExamine=0) 
	  ...
	  For count=1 To 12
	   If foc=count 
	    Repeat
	     charMove(0,count)=charMove(0,count)-1
	     If charMove(0,count)<1 Then charMove(0,count)=moveList(1)
	    Until moveLearned(1,charMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf
	 ;search right
	 If KeyDown(205) Or JoyXDir()=1 Or (MouseDown(1) And MouseX()>rX#(400) And editExamine=0)
	  ...
	  For count=1 To 12
	   If foc=count
	    Repeat
	     charMove(0,count)=charMove(0,count)+1
	     If charMove(0,count)>moveList(1) Then charMove(0,count)=1
	    Until moveLearned(1,charMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf
	 ...
	 If KeyDown(57)
	  ...
	  For count=1 To 12
	   If foc=count
	    Repeat
	     charMove(0,count)=Rnd(1,moveList(1))
	    Until moveLearned(1,charMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	 EndIf
	 ...
	 If KeyDown(14)
	  ...
	  For count=1 To 12
	   If foc=count
	    If moveLearned(1,7)>0 Or game=0 Or screenAgenda=5 Then charMove(0,count)=7
	   EndIf
	  Next
	 EndIf
	EndIf 
	...
	If page=5 And blockMoves=0 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or (MouseDown(2) And MouseX()>rX#(400) And editExamine=0)  
	  ...
	  For count=1 To 3
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=charGroundMove(0,count)-1 
	     If charGroundMove(0,count)<1 Then charGroundMove(0,count)=moveList(3)
	    Until moveLearned(3,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=4 To 6
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=charGroundMove(0,count)-1 
	     If charGroundMove(0,count)<1 Then charGroundMove(0,count)=moveList(4)
	    Until moveLearned(4,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  If foc=7 Or foc=9
	   Repeat
	    charMove(0,foc+6)=charMove(0,foc+6)-1 
	    If charMove(0,foc+6)<1 Then charMove(0,foc+6)=moveList(1)
	   Until moveLearned(1,charMove(0,foc+6))>0 Or game=0 Or screenAgenda=5
	  EndIf
	  If foc=8
	   Repeat
	    charMove(0,14)=charMove(0,14)-1 
	    If charMove(0,14)<1 Then charMove(0,14)=moveList(2)
	   Until moveLearned(2,charMove(0,14))>0 Or game=0 Or screenAgenda=5
	  EndIf
	 EndIf
	 ;search right
	 If KeyDown(205) Or JoyXDir()=1 Or (MouseDown(1) And MouseX()>rX#(400) And editExamine=0)
      ...
      For count=1 To 3
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=charGroundMove(0,count)+1 
	     If charGroundMove(0,count)>moveList(3) Then charGroundMove(0,count)=1
	    Until moveLearned(3,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=4 To 6
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=charGroundMove(0,count)+1 
	     If charGroundMove(0,count)>moveList(4) Then charGroundMove(0,count)=1
	    Until moveLearned(4,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  If foc=7 Or foc=9
	   Repeat
	    charMove(0,foc+6)=charMove(0,foc+6)+1 
	    If charMove(0,foc+6)>moveList(1) Then charMove(0,foc+6)=1
	   Until moveLearned(1,charMove(0,foc+6))>0 Or game=0 Or screenAgenda=5
	  EndIf
	  If foc=8
	   Repeat
	    charMove(0,14)=charMove(0,14)+1 
	    If charMove(0,14)>moveList(2) Then charMove(0,14)=1
	   Until moveLearned(2,charMove(0,14))>0 Or game=0 Or screenAgenda=5
	  EndIf
	 EndIf
	 ;randomize
	 If KeyDown(57)
	  ...
	  For count=1 To 3
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=Rnd(1,moveList(3))
	    Until moveLearned(3,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  For count=4 To 6
	   If foc=count
	    Repeat
	     charGroundMove(0,count)=Rnd(1,moveList(4))
	    Until moveLearned(4,charGroundMove(0,count))>0 Or game=0 Or screenAgenda=5
	   EndIf
	  Next
	  If foc=7 Or foc=9
	   Repeat
	    charMove(0,foc+6)=Rnd(1,moveList(1))
	   Until moveLearned(1,charMove(0,foc+6))>0 Or game=0 Or screenAgenda=5
	  EndIf
	  If foc=8
	   Repeat
	    charMove(0,14)=Rnd(1,moveList(2))
	   Until moveLearned(2,charMove(0,14))>0 Or game=0 Or screenAgenda=5
	  EndIf
	 EndIf
	 ;revert
	 If KeyDown(14)
	  ...
	  For count=1 To 3
	   If foc=count
	    If moveLearned(3,1)>0 Or game=0 Or screenAgenda=5 Then charGroundMove(0,count)=1
	   EndIf
	   If foc=count+3
	    If moveLearned(4,1)>0 Or game=0 Or screenAgenda=5 Then charGroundMove(0,count+3)=1
	   EndIf
	  Next
	  If foc=7
	   If moveLearned(1,8)>0 Or game=0 Or screenAgenda=5 Then charMove(0,13)=moveList(1)
	  EndIf
	  If foc=8
	   If moveLearned(2,1)>0 Or game=0 Or screenAgenda=5 Then charMove(0,14)=1
	  EndIf
	  If foc=9
	   If moveLearned(1,8)>0 Or game=0 Or screenAgenda=5 Then charMove(0,15)=moveList(1)
	  EndIf
	 EndIf
	EndIf 
	...
	If page=>6 And page=<8 And blockCostume=0 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or (MouseDown(2) And MouseX()>rX#(400) And editExamine=0)
      ...
      If foc=5 
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charBody(0,coz)=charBody(0,coz)-1 
        If charBody(0,coz)<1 Then charBody(0,coz)=no_bodies
       Until costumeLearned(1,charBody(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
      If foc=6
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charRightArm(0,coz)=charRightArm(0,coz)-1
        If charRightArm(0,coz)<1 Then charRightArm(0,coz)=no_arms
       Until costumeLearned(2,charRightArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charRightForearm(0,coz)=charRightArm(0,coz) : charLeftArm(0,coz)=charRightArm(0,coz) : charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=7
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charRightForearm(0,coz)=charRightForearm(0,coz)-1
        If charRightForearm(0,coz)<1 Then charRightForearm(0,coz)=no_arms
       Until costumeLearned(2,charRightForearm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charRightForearm(0,coz)
      EndIf
      If foc=8
       PlaySound sMenuBrowse : keytim=5
      ...
      If foc=9
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charLeftArm(0,coz)=charLeftArm(0,coz)-1
        If charLeftArm(0,coz)<1 Then charLeftArm(0,coz)=no_arms
       Until costumeLearned(2,charLeftArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=10
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charLeftForearm(0,coz)=charLeftForearm(0,coz)-1
        If charLeftForearm(0,coz)<1 Then charLeftForearm(0,coz)=no_arms
       Until costumeLearned(2,charLeftForearm(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf 
      ...
      If foc=12
       ...
       Until (costumeLearned(3,charShorts(0,coz))>0 Or charShorts(0,coz)=0 Or game=0 Or screenAgenda=5) And ShortsViable(charShorts(0,coz))
      EndIf
      If foc=13
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charLegs(0,coz)=charLegs(0,coz)-1
        If charLegs(0,coz)<1 Then charLegs(0,coz)=no_legs
       Until costumeLearned(3,charLegs(0,coz))>0 Or game=0 Or screenAgenda=5
       charShins(0,coz)=charLegs(0,coz) : charShoes(0,coz)=charLegs(0,coz)
      EndIf
	  If foc=14
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charShins(0,coz)=charShins(0,coz)-1
        If charShins(0,coz)<1 Then charShins(0,coz)=no_legs
       Until costumeLearned(3,charShins(0,coz))>0 Or game=0 Or screenAgenda=5
       charShoes(0,coz)=charShins(0,coz)
      EndIf
      If foc=15
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charShoes(0,coz)=charShoes(0,coz)-1
        If charShoes(0,coz)<1 Then charShoes(0,coz)=no_legs
       Until costumeLearned(3,charShoes(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
	 EndIf
	 ;search right
	 If KeyDown(205) Or JoyXDir()=1 Or (MouseDown(1) And MouseX()>rX#(400) And editExamine=0)
      ...
      If foc=5
       ...
       Repeat
        charBody(0,coz)=charBody(0,coz)+1 
        If charBody(0,coz)>no_bodies Then charBody(0,coz)=1
       Until costumeLearned(1,charBody(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
      If foc=6
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charRightArm(0,coz)=charRightArm(0,coz)+1
        If charRightArm(0,coz)>no_arms Then charRightArm(0,coz)=1
       Until costumeLearned(2,charRightArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charRightForearm(0,coz)=charRightArm(0,coz) : charLeftArm(0,coz)=charRightArm(0,coz) : charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=7
       ...
       Repeat
        charRightForearm(0,coz)=charRightForearm(0,coz)+1
        If charRightForearm(0,coz)>no_arms Then charRightForearm(0,coz)=1
       Until costumeLearned(2,charRightForearm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charRightForearm(0,coz)
      EndIf
      If foc=8
      ...
      If foc=9
       ...
       Repeat
        charLeftArm(0,coz)=charLeftArm(0,coz)+1
        If charLeftArm(0,coz)>no_arms Then charLeftArm(0,coz)=1
       Until costumeLearned(2,charLeftArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=10
       ...
       Repeat
        charLeftForearm(0,coz)=charLeftForearm(0,coz)+1
        If charLeftForearm(0,coz)>no_arms Then charLeftForearm(0,coz)=1
       Until costumeLearned(2,charLeftForearm(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf 
      If foc=11 Then charLeftHand(0,coz)=charLeftHand(0,coz)+1 : PlaySound sMenuBrowse : keytim=5
      If foc=12
       ...
       Repeat
        charShorts(0,coz)=charShorts(0,coz)+1
        If charShorts(0,coz)>no_legs Then charShorts(0,coz)=0
       Until (costumeLearned(3,charShorts(0,coz))>0 Or charShorts(0,coz)=0 Or game=0 Or screenAgenda=5) And ShortsViable(charShorts(0,coz))
      EndIf
      If foc=13
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charLegs(0,coz)=charLegs(0,coz)+1
        If charLegs(0,coz)>no_legs Then charLegs(0,coz)=1
       Until costumeLearned(3,charLegs(0,coz))>0 Or game=0 Or screenAgenda=5
       charShins(0,coz)=charLegs(0,coz) : charShoes(0,coz)=charLegs(0,coz)
      EndIf
	  If foc=14
       ...
       Repeat
        charShins(0,coz)=charShins(0,coz)+1
        If charShins(0,coz)>no_legs Then charShins(0,coz)=1
       Until costumeLearned(3,charShins(0,coz))>0 Or game=0 Or screenAgenda=5
       charShoes(0,coz)=charShins(0,coz)
      EndIf
      If foc=15
       ...
       Repeat
        charShoes(0,coz)=charShoes(0,coz)+1
        If charShoes(0,coz)>no_legs Then charShoes(0,coz)=1
       Until costumeLearned(3,charShoes(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
	 EndIf 
	 ;randomize
	 If KeyDown(57)
      ...
      If foc=5 
       Repeat
        charBody(0,coz)=Rnd(1,no_bodies)
       Until costumeLearned(1,charBody(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
      If foc=6
       PlaySound sMenuBrowse : keytim=5
       Repeat
        charRightArm(0,coz)=Rnd(1,no_arms)
       Until costumeLearned(2,charRightArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charRightForearm(0,coz)=charRightArm(0,coz) : charLeftArm(0,coz)=charRightArm(0,coz) : charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=7
       Repeat
        charRightForearm(0,coz)=Rnd(1,no_arms)
       Until costumeLearned(2,charRightForearm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charRightForearm(0,coz)
      EndIf
      If foc=8
       charRightHand(0,coz)=Rnd(1,no_hands) : charLeftHand(0,coz)=charRightHand(0,coz)
      EndIf
      If foc=9
       Repeat
        charLeftArm(0,coz)=Rnd(1,no_arms)
       Until costumeLearned(2,charLeftArm(0,coz))>0 Or game=0 Or screenAgenda=5
       charLeftForearm(0,coz)=charLeftArm(0,coz)
      EndIf
      If foc=10
       Repeat
        charLeftForearm(0,coz)=Rnd(1,no_arms)
       Until costumeLearned(2,charLeftForearm(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf 
      If foc=11 Then charLeftHand(0,coz)=Rnd(1,no_hands)
      If foc=12
       Repeat
        charShorts(0,coz)=Rnd(1,no_legs)
       Until (costumeLearned(3,charShorts(0,coz))>0 Or charShorts(0,coz)=0 Or game=0 Or screenAgenda=5) And ShortsViable(charShorts(0,coz))
      EndIf
      If foc=13
       Repeat
        charLegs(0,coz)=Rnd(1,no_legs)
       Until costumeLearned(3,charLegs(0,coz))>0 Or game=0 Or screenAgenda=5
       charShins(0,coz)=charLegs(0,coz) : charShoes(0,coz)=charLegs(0,coz)
      EndIf
	  If foc=14
       Repeat
        charShins(0,coz)=Rnd(1,no_legs)
       Until costumeLearned(3,charShins(0,coz))>0 Or game=0 Or screenAgenda=5
       charShoes(0,coz)=charShins(0,coz)
      EndIf
      If foc=15
       Repeat
        charShoes(0,coz)=Rnd(1,no_legs)
       Until costumeLearned(3,charShoes(0,coz))>0 Or game=0 Or screenAgenda=5
      EndIf
	 EndIf  
	 ;revert
	 If KeyDown(14)
      ...
      If foc=5 Then charBody(0,coz)=charBody(editChar,coz)
      If foc=6
       charRightArm(0,coz)=charRightArm(editChar,coz) : charRightForearm(0,coz)=charRightArm(0,coz)
      ...
      If foc=9 Then charLeftArm(0,coz)=charLeftArm(editChar,coz) : charLeftForearm(0,coz)=charLeftArm(0,coz)
      ...
      If foc=13 Then charLegs(0,coz)=charLegs(editChar,coz) : charShins(0,coz)=charLegs(0,coz) : charShoes(0,coz)=charLegs(0,coz)
	...
	If game=1 And charClause(gamChar,1)=<1 Then charWeapon(0)=0
	If game=1 And charClause(gamChar,1)=2 Then editGimmickChanges=GimmickExpenses()
	;scale
 ...
 If page=9
  ...
  If charEyeShape(0)=112 Then namer$="Crossed"
 ...
 EndIf
 ;costume expenses
 forecast=gamGimmickChanges+editGimmickChanges
 If charWeapon(0)>0 And gamAgreement(17)=0 Then forecast=forecast+weapValue(charWeapon(0))
 If go=0 And game=1 And forecast>0
  x=rX#(80) : y=rY#(25)
  SetFont font(3)
  Outline("Gimmick Changes:",x,y-10,0,0,0,255,240,100)
  SetFont fontNews(7)
  r=255 : g=255 : b=255
  If forecast>charBank(gamChar) Then r=Rnd(175,255) : g=0 : b=0
  Outline("$"+GetFigure$(forecast),x,y+10,0,0,0,r,g,b)
 EndIf
 ;loading call
 If screenCall=1
...
If go>0
 ...
 ;If GraphicsWidth()>1024 Then photoX#=photoX#+60
 ;charPhoto(editChar)=CreateImage(rY#(330),rY#(540))
charPhoto(editChar)=CreateImage(rY#(1),rY#(1))
 ...
 ResizeImage charPhoto(editChar),1,1
 SaveImage(charPhoto(editChar),"Data/Slot0"+slot+"/Portraits/Photo"+Dig$(editChar,100)+".bmp")
 ;translate changes 
 gamGimmickChanges=gamGimmickChanges+editGimmickChanges 
 CopyChar(0,editChar)
 ...
 If screenAgenda=5 Then ResetOldValues(editChar) : gamGimmickChanges=0
 If game=1 Then LearnMoves(0,2) : LearnCostumes(0,2)
 ;save changes
...
Function ApplyHairstyle(cyc,headwear)
 ...
 If charHairStyle(char,coz)=49 Then showHairA$="H_Short" : showHairB$="H_Slick"
...
Function ApplyClothing(cyc)
 ...
 EndIf
 ;learn costumes
 If game=1 And screen<>51
  If costumeLearned(1,charBody(char,coz))<1 Then costumeLearned(1,charBody(char,coz))=1
  If costumeLearned(2,charLeftArm(char,coz))<1 Then costumeLearned(2,charLeftArm(char,coz))=1
  If costumeLearned(2,charLeftForearm(char,coz))<1 Then costumeLearned(2,charLeftForearm(char,coz))=1
  If costumeLearned(2,charRightArm(char,coz))<1 Then costumeLearned(2,charRightArm(char,coz))=1
  If costumeLearned(2,charRightForearm(char,coz))<1 Then costumeLearned(2,charRightForearm(char,coz))=1
  If costumeLearned(3,charShorts(char,coz))<1 Then costumeLearned(3,charShorts(char,coz))=1
  If costumeLearned(3,charLegs(char,coz))<1 Then costumeLearned(3,charLegs(char,coz))=1
  If costumeLearned(3,charShins(char,coz))<1 Then costumeLearned(3,charShins(char,coz))=1
  If costumeLearned(3,charShoes(char,coz))<1 Then costumeLearned(3,charShoes(char,coz))=1
 EndIf
 ;remove limbs
 For limb=1 To 50
...
Function EditingAdvice(x,y)
 ...
 If page=1
  If foc=<2 And blockName=1
   ...
   If game=1 And gamAgreement(1)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
 ...
 ;gimmick blocks
 If page=2
  If foc=1 Or foc=12 Or foc=13
   If blockData=1 Or screenAgenda=5 Then warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
  EndIf
  If foc=2 And blockAllegiance=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(8)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
  If foc=>3 And foc=<7 And blockTaunts=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(7)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
  If foc=>8 And foc=<10 And blockEntrance=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(4)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
  If foc=11 And game=1 And gamAgreement(17)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
 EndIf
 ;attack blocks
 If page=3
  If foc=>1 And foc=<9 And blockAttacks=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(5)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
 EndIf
 ;move blocks
 If page=4
  If foc=>1 And foc=<12 And blockMoves=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(6)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
 EndIf
 If page=5
 ...
 If page=>6 And page=<8
  If foc=>1 And foc=<15 And blockCostume=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(3)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
 EndIf
 ;appearance blocks
 If page=9
  ...
  If foc=>4 And foc=<5 And blockHair=1
   warnX=x : warnY=y+13 : warning$="(This value is beyond your control)"
   If game=1 And gamAgreement(2)>0 Then warnX=x : warnY=y+13 : warning$="(You agreed to change this value)"
  EndIf
...
Function GimmickExpenses()
 value=0
 If screenAgenda<>5 And screenAgenda<>9
  ;gimmicks
  ...
  If charBeard(0,1)>0 And charBeard(0,1)<>charBeard(gamChar,1) Then value=value+25
  If charTattoos(0)>0 And charTattoos(0)<>charTattoos(gamChar) Then value=value+25
  For count=1 To 3
   ...
   If charLeftForearm(0,count)<>charLeftArm(0,count) And charLeftForearm(0,count)<>charLeftForearm(gamChar,count) Then value=value+25
   If charRightForearm(0,count)<>charRightArm(0,count) And charRightForearm(0,count)<>charRightForearm(gamChar,count) Then value=value+25
   If charLeftHand(0,count)<>charLeftHand(gamChar,count) Then value=value+25
   If charRightHand(0,count)<>charRightHand(gamChar,count) Then value=value+25
   ...
   If charShins(0,count)<>charLegs(0,count) And charShins(0,count)<>charShins(gamChar,count) Then value=value+25
...
;AUTOMATICALLY LEARN GIVEN CHARACTER'S MOVES
Function LearnMoves(char,level)
 ;moves
 For count=1 To 13
  If moveLearned(1,charMove(char,count))=0 Then moveLearned(1,charMove(char,count))=level
 Next
 If moveLearned(2,charMove(char,14))=0 Then moveLearned(2,charMove(char,14))=level
 If moveLearned(1,charMove(char,15))=0 Then moveLearned(1,charMove(char,15))=level
 ;ground moves
 For count=1 To 3
  If moveLearned(3,charGroundMove(char,count))=0 Then moveLearned(3,charGroundMove(char,count))=level
  If moveLearned(4,charGroundMove(char,count+3))=0 Then moveLearned(4,charGroundMove(char,count+3))=level
 Next 
 ;attacks
 For count=1 To 5
  If attackLearned(count,charAttack(char,count))=0 Then attackLearned(count,charAttack(char,count))=level
  If crushLearned(count,charCrush(char,count))=0 Then crushLearned(count,charCrush(char,count))=level
 Next
 ;gestures
 For count=1 To 4
  If tauntLearned(charTaunt(char,count))=0 Then tauntLearned(charTaunt(char,count))=level
 Next
End Function

;AUTOMATICALLY LEARN GIVEN CHARACTER'S COSTUMES
Function LearnCostumes(char,level)
 ;costumes
 For count=1 To 3
  If costumeLearned(1,charBody(char,count))=0 Then costumeLearned(1,charBody(char,count))=level
  If costumeLearned(2,charLeftArm(char,count))=0 Then costumeLearned(2,charLeftArm(char,count))=level
  If costumeLearned(2,charRightArm(char,count))=0 Then costumeLearned(2,charRightArm(char,count))=level
  If costumeLearned(3,charLegs(char,count))=0 Then costumeLearned(3,charLegs(char,count))=level
  If costumeLearned(3,charShorts(char,count))=0 Then costumeLearned(3,charShorts(char,count))=level
  If costumeLearned(3,charShoes(char,count))=0 Then costumeLearned(3,charShoes(char,count))=level
 Next
 ;music
 If musicLearned(charTheme(char))=0 Then musicLearned(charTheme(char))=level
End Function