...
Function PrepareWeapons()
 ...
 For cyc=1 To no_plays
  ;default weapon
  pWeapon(cyc)=0
  If game=1 And gamAgreement(19)>0 Then weapBlock=1 Else weapBlock=0
  If weapBlock=0 And charWeapon(pChar(cyc))>0 And pRole(cyc)<>1 Then pWeapon(cyc)=charWeapon(pChar(cyc))
  ;promo mic
  If weapBlock=0 And matchPromo>0 And matchPromo<>74 And matchLocation=0 And fed<>7
   If cyc=promoActor(1) Or cyc=promoActor(2) Or cyc=promoActor(3) Then pWeapon(cyc)=5
  EndIf
  ;star player's prop
  If game=1 And pChar(cyc)=gamChar And charWeapon(pChar(cyc))>0 And pRole(cyc)<>1 Then pWeapon(cyc)=charWeapon(pChar(cyc))
  ;cup holder
...
;new belts
;Womens
If TitleHolder(pChar(cyc),5) And matchChamps<>3 And pRole(cyc)<>1 And (screenAgenda<>11 Or cupReward(cupSlot)<>5)
   pWeapon(cyc)=39
   If matchChamps=1 And matchTeams=<0 And pRole(cyc)=0 Then matchReward=8
  EndIf
;US
If TitleHolder(pChar(cyc),6) And matchChamps<>3 And pRole(cyc)<>1 And (screenAgenda<>11 Or cupReward(cupSlot)<>6)
   pWeapon(cyc)=40
   If matchChamps=1 And matchTeams=<0 And pRole(cyc)=0 Then matchReward=9
  EndIf
;TV
If TitleHolder(pChar(cyc),7) And matchChamps<>3 And pRole(cyc)<>1 And (screenAgenda<>11 Or cupReward(cupSlot)<>7)
   pWeapon(cyc)=41
   If matchChamps=1 And matchTeams=<0 And pRole(cyc)=0 Then matchReward=10
  EndIf

;end
  ;force product placement
  If matchPromo=29 And charWeapon(pChar(cyc))>0 Then pWeapon(cyc)=charWeapon(pChar(cyc))
  ;add to list
...
;new belts
If TitleHolder(pChar(cyc),5) Then reward=39
  If TitleHolder(pChar(cyc),6) Then reward=40
  If TitleHolder(pChar(cyc),7) Then reward=41
;end
