;-------------------------------------------------------------------------
;///////////////// WRESTLING MPIRE REMIX: CAREER EDITION /////////////////
;-------------------------------------------------------------------------
...
Function LoadScreen(request)
 ...
 ;If request=55 Then PressConference()
 If request=56 Then ArrangeMatch()
 If request=57 Then Recruitment()
End Function
...
If game=1
 If screenAgenda=12 Or screenAgenda=13 Or screenAgenda=14 Then optLength=1
...
;new belts
fedOldChampWomens(promotion)=fedChampWomens(promotion)
 fedOldChampUS(promotion)=fedChampUS(promotion)
fedOldChampTV(promotion)=fedChampTV(promotion)
 ;end
 For count=1 To 2
 ...
 For framer=1 To frames
  ...
  If gotim>40 And keytim=0 
   If KeyDown(1) Or KeyDown(28) 
    ...
    EndIf
    ;skip promos
	If matchState=2 And promoTim>25 And promoTim<promoLength(matchPromo)-25 
	 promoTim=promoTim-100 : keytim=10 
	 If promoTim<25 Then promoTim=25
	EndIf
	;abort match 
    abortBlock=0 
    If game=1 And screenAgenda=>13 And screenAgenda=<14 Then abortBlock=1
    If KeyDown(1) And abortBlock=0 And matchState=3 And matchTim>50
     winner=0
     For v=1 To no_wrestlers
      If matchPlayer>0 And pTeam(v)<>pTeam(matchPlayer) And pControl(v)=0 And pOutTim(v)=>2 And pHidden(v)=0 And pEliminated(v)=0 Then winner=v
     Next
     If matchPlayer>0 Then matchLoser=matchPlayer
     If pEliminated(matchPlayer) Then matchWinStyle=6 Else matchWinStyle=0
     EndMatch(winner)
     keytim=20
    EndIf 
	;exit
  ...
  If matchPause=0  
   ...
   If gotim>40 And keytim=0
    ...
    ;restart match!
    If KeyDown(56) And KeyDown(211) And matchState=4 And matchCountOuts<3
    ...
    EndIf
    ;switch character
    If KeyDown(15) And game=0 And optOnline=0
     newbie=matchPlayer
     Repeat 
...
;new belts
     If matchReward=8 Then namer$="Womens Championship on the line!" 
     If matchReward=9 Then namer$="US Championship on the line!" 
     If matchReward=10 Then namer$="TV Championship on the line!"
;end 
    Else
   ...
   If active=1
    For count=1 To 10
     If FindChild(world,"Screen"+Dig$(count,10))>0
      ;change image
     randy=Rnd(-1,1500)
     If randy=>-1 And randy=<7 Then videoScreen(count)=Rnd(1,6)
 ...
 ;If diagnose=1 Or KeyDown(42)
  ;SetFont fontStat(1)
  ;Outline("X: "+pX#(1)+" ("+RealX#(1)+")",100,150,0,0,0,255,255,255)
  ;Outline("Y: "+pY#(1)+" ("+RealY#(1)+")",100,165,0,0,0,255,255,255)
  ;Outline("Z: "+pZ#(1)+" ("+RealZ#(1)+")",100,180,0,0,0,255,255,255) 
  ;Outline("pA#: "+pA#(1)+" ("+pA#(2)+")",100,195,0,0,0,255,255,255) 
  ;Outline("Body Distance: "+GetDistance#(pX#(1),pZ#(1),pX#(2),pZ#(2)),100,250,0,0,0,255,255,255) 
  ;Outline("Head Distance: "+GetDistance#(pX#(1),pZ#(1),pLimbX#(2,1),pLimbZ#(2,1)),100,265,0,0,0,255,255,255)  
 ;EndIf
 ;mask shaky start
 ...
 If arenaCrowd>0 And optCrowdAnim=>1
  For cyc=1 To 200
   If FindChild(world,"Crowd"+Dig$(cyc,100))>0
    ...
    EndIf
    If KeyDown(66) Then optCrowdAnim=0
   EndIf
  Next
 EndIf
End Function
...
Function EndMatch(cyc)
 If matchState<>4
  ...
  Pop(matchWinner,Rnd(2,9),1)
  If matchWinner>0 And screenAgenda<>12
   If fed<>7 Or arenaPreset=>11 Then PlayTheme(pChar(matchWinner))
  EndIf
  ...
  For v=1 To no_plays
   ...
   EndIf
   ;leave promotion
   If matchReward=7 And matchWinner>0 And pTeam(v)<>pTeam(matchWinner) And pRole(v)=0 Then MoveChar(pChar(v),7)
  Next
...
;new belts DEV
If matchReward=8 Then fedChampWomens(fed)=pChar(matchWinner) : WriteHistory(fed,5)
   If matchReward=9 Then fedChampUS(fed)=pChar(matchWinner) : WriteHistory(fed,6)
If matchReward=10 Then fedChampTV(fed)=pChar(matchWinner) : WriteHistory(fed,7)
  ;end
...
;new belts DEV
If TitleHolder(pChar(matchWinner),8) Then comSuffix$="Womens Champion"
If TitleHolder(pChar(matchWinner),9) Then comSuffix$="US Champion"
If TitleHolder(pChar(matchWinner),10) Then comSuffix$="TV Champion"
...
;new belts
If TitleHolder(pChar(matchWinner),8) And pChar(matchWinner)<>fedOldChampWomens(fed) Then comSuffix$="New Womens Champion!"
If TitleHolder(pChar(matchWinner),9) And pChar(matchWinner)<>fedOldChampUS(fed) Then comSuffix$="New US Champion!"
If TitleHolder(pChar(matchWinner),10) And pChar(matchWinner)<>fedOldChampTV(fed) Then comSuffix$="New TV Champion!"
;end
   EndIf
   If matchTeams>0 
