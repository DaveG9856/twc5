...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or MouseDown(2)
	  ...
	  If foc=2 Then optTrainLevel=optTrainLevel-1 : PlaySound sMenuBrowse : keytim=5
	...
	If optTrainLevel<0 Then optTrainLevel=0
	If optTrainLevel>3 Then optTrainLevel=3 
 ...
 DrawOption(2,rX#(400),rY#(y),"Training System",textTrainLevel$(optTrainLevel)) : y=y+32
...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or MouseDown(2)
	  ...
	  If foc=6 Then optContent=optContent-1 : PlaySound sMenuBrowse : keytim=5 
	  If foc=7 Then optFX=optFX-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=8 Then optShadows=optShadows-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=9 Then optGore=optGore-1 : PlaySound sMenuBrowse : keytim=5 
	  If foc=10 Then optMeters=optMeters-1 : PlaySound sMenuBrowse : keytim=5 
	...
	If optContent<0 Then optContent=0
	If optContent>2 Then optContent=2 
 ...
 DrawOption(3,rX#(400),rY#(y),"Fog Effect",textOnOff$(optFog)) : y=y+33
 DrawOption(4,rX#(400),rY#(y),"Arena Detail",textDetail$(optDetail)) : y=y+33 
 DrawOption(5,rX#(400),rY#(y),"Crowd",textCrowdAnim$(optCrowdAnim)) : y=y+33 
 DrawOption(6,rX#(400),rY#(y),"Career Content",textContent$(optContent)) : y=y+36 
 DrawOption(7,rX#(400),rY#(y),"Particle FX",textFX$(optFX)) : y=y+33
 DrawOption(8,rX#(400),rY#(y),"Shadows",textShadows$(optShadows)) : y=y+33  
 DrawOption(9,rX#(400),rY#(y),"Gore",textGore$(optGore)) : y=y+36
 DrawOption(10,rX#(400),rY#(y),"Health Meters",textMeters$(optMeters)) : y=y+33
...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And keytim=0
	 ...
	 If KeyDown(28) Or ButtonPressed() Or MouseDown(1)
	  ...
	  ;block access
	  If go=1 And screenAgenda=1 And foc=>1 And foc=<9
	...
	If gotim>20 And keytim=0
	 ...
	 If screenAgenda=1 And KeyDown(56) And KeyDown(36)
      ...
      For char=1 To no_chars
       ...
       ;new belts
       If fedSize(charFed(char))=3 And charFed(char)=<6 Then fedChampWomens(charFed(char))=char
       If fedSize(charFed(char))=3 And charFed(char)=<6 Then fedChampUS(charFed(char))=char
       If fedSize(charFed(char))=3 And charFed(char)=<6 Then fedChampTV(charFed(char))=char
       ;end
	 ...
	 If screenAgenda=1 And KeyDown(17) And KeyDown(50)
 ...
 For count=1 To 9 
  ...
  If foc<>count Or (screenAgenda=1 And fedLocked(count)) 
  ...
  If screenAgenda=1 And fedLocked(count) 
   ...
   If foc=count And flashTim=<20
    If count=>7 Then namer$="Win All World Titles!" Else namer$="Win The World Title!"
 ...
 If foc<>oldfoc Then oldfoc=foc : PlaySound sMenuSelect  
 DrawImage gCursor,MouseX(),MouseY()
 If (screenAgenda=>3 And screenAgenda=<4) Or screenAgenda=6 Then DrawImage charPhoto(gamChar),MouseX()+40,MouseY()+25
 If screenAgenda=>7 And screenAgenda=<8 Then DrawImage charPhoto(editChar),MouseX()+40,MouseY()+25
MaskImage charPhoto(editChar),24,4,0
 Flip
 ;screenshot (F12)
 If KeyHit(88) Then Screenshot()
...
If screenAgenda=2 ;studying in career
 ...
 If go=1 Then screen=12
EndIf
If screenAgenda=4 ;arrange match
 If go=-1 Then screen=20
 If go=1 Then screen=12
EndIf
...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And keytim=0
	 ...
	 If KeyDown(28) Or ButtonPressed() Or (MouseDown(1) And MouseOverBox())
	  ...
	  EndIf
	  If game=1 And fedRank(fed,foc)=gamChar
	   If screenAgenda=3 Or screenAgenda=4 Or screenAgenda=6 Then BlockAccess()
	  EndIf
	 EndIf
	 ;switch display
	 If KeyDown(15)
	  ...
	  If optGrid=0 And editFilter=0 Then editFilter=1 : GetRankings(editFilter)
	  If optGrid=1 And editFilter<>0 Then editFilter=0 : GetRankings(editFilter)
	  scroll=0
	 EndIf
	EndIf
	;music
    ...
    Next
    ;add one for editing
    If screenAgenda=1 And fed>0 And no_chars<optCharLim And fedSize(fed)<optRosterLim
     no_boxes=fedSize(fed)+1
     charName$(no_chars+1)="~ ADD CHARACTER ~"
     fedRoster(fed,no_boxes)=no_chars+1
     fedRank(fed,no_boxes)=no_chars+1
     charPhoto(no_chars+1)=charPhoto(0)
    EndIf
    ;identify career player
    If game=1 And gotim=1
     For cyc=1 To no_boxes
      If fedRank(fed,cyc)=gamChar Then foc=cyc
     Next
    EndIf
	...
	If gotim>20 And keytim=0
	 ...
	 If screenAgenda=1 And foc>0 And char=<no_chars
	  ;switch rosters
	  If fedSize(charFed(char))>3 Or (fedSize(charFed(char))>1 And charFed(char)=>8)
	   For count=1 To 9
	    If KeyDown(count+1) And fedSize(count)<optRosterLim And charFed(char)<>count
	     If count=<7 Or char<>gamChar Or slot=0
	  ...
	  EndIf
	  ;change titles
	  If KeyDown(48) And fedBooker(charFed(char))<>char
	   If char<>gamChar Or slot=0 Then fedBooker(charFed(char))=char : PlaySound sPaper : keytim=10
	  EndIf
	  If charFed(char)=<6
	   ...
	   ;new belts
	   If KeyDown(35) And fedChampWomens(charFed(char))<>char Then fedChampWomens(charFed(char))=char : PlaySound sPaper : keytim=10
	   If KeyDown(36) And fedChampUS(charFed(char))<>char Then fedChampUS(charFed(char))=char : PlaySound sPaper : keytim=10
	   If KeyDown(37) And fedChampTV(charFed(char))<>char Then fedChampTV(charFed(char))=char : PlaySound sPaper : keytim=10
	   ;end
 ...
 If no_boxes>no_displays 
  If scroll<0
   ...
   If MouseY()>rY#(y)-20 And MouseY()<rY#(y)+20
    If MouseX()>(rX#(400)-360)-15 And MouseX()<(rX#(400)-360)+20 Then subFoc=20
    If MouseX()>(rX#(400)+360)-15 And MouseX()<(rX#(400)+360)+20 Then subFoc=20
   EndIf
   If subFoc=20
 ...
 EndIf
 ;CHARACTER BOXES
 If optGrid=0 Then x=rX#(400)-195 : listY=rY#(150)
 ...
 For rank=1 To no_boxes
  ...
  If char>0 And y=>rY#(150) And y=<rY#(510)
   ...
   Else
    If MouseX()=>x-155 And MouseX()=<x+525 And MouseY()=>y-15 And MouseY()=<y+14
     foc=rank
     ...
     For count=1 To 11
      ...
      statX=statX+29
     Next
     If MouseX()=>x+417 Then subFoc=12
    EndIf
   EndIf
   ;rank
   ...
   EndIf
   ;name box
   If foc=rank And subFoc<20 Then DrawImage gMenu(5),x,y+1 Else DrawImage gMenu(6),x,y+1
   ...
   If foc=rank And subFoc<20 Then DrawImage gMenu(7),x,y+1 Else DrawImage gMenu(8),x,y+1
   DrawImage gPortrait,x,y+1
   DrawImageRect charPhoto(char),x-48,(y+64)-11,28,(64-PortraitHead(char))-10,21,21
   MaskImage charPhoto(char),24,4,0
 ...
 If (screenAgenda=>3 And screenAgenda=<4) Or screenAgenda=6 Then DrawImage charPhoto(gamChar),MouseX()+40,MouseY()+25
 If screenAgenda=>7 And screenAgenda=<8 Then DrawImage charPhoto(editChar),MouseX()+40,MouseY()+25
MaskImage charPhoto(gamChar),24,4,0
 Flip
 ;screenshot (F12)
 If KeyHit(88) Then Screenshot()
...
If screenAgenda=2 ;studying
 ...
 If go=1 Then screen=11
EndIf
If screenAgenda=3 ;hire manager
 If go=-1 Then screen=20
 If go=1 
  negTopic=3 : negChar=fedRank(fed,foc)
  ;If charClause(gamChar,1)<2 Then negTopic=-2 : negChar=fedBooker(fed)
  screen=57
 EndIf
EndIf
If screenAgenda=4 ;arrange match
 If go=-1 Then screen=20
 If go=1 
  negTopic=1 : negChar=fedRank(fed,foc) : screen=56
  If charClause(gamChar,1)<2 Then negTopic=-3 : negChar=fedBooker(fed) : screen=57
 EndIf
EndIf
If screenAgenda=6 ;recruit partner
 If go=-1 Then screen=20
 If go=1
  negTopic=1 : negChar=fedRank(fed,foc)
  If charClause(gamChar,1)<2 Then negTopic=-1 : negChar=fedBooker(fed)
  screen=57
 EndIf
EndIf
If screenAgenda=7 Or screenAgenda=8 ;setting relationships
...
While go=0
 ...
 For framer=1 To frames
	...
	EndIf
	;check limits
	If matchPreset>no_matches Then matchPreset=1
	If matchPreset<1 Then matchPreset=no_matches
 ...
 For framer=1 To frames
	...
	If keytim=0
	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or MouseDown(2)
	  ...
	  If foc=3 Then matchBlastTim=matchBlastTim-1 : keytim=3
	  ...
	  If foc=10
	   Repeat  
	    ...
	    If matchChamps<0 Then matchChamps=10 ;new belts orig = 7
	 ...
	 If KeyDown(205) Or JoyXDir()=1 Or MouseDown(1)
	  ...
	  If foc=10
	   Repeat  
	    matchChamps=matchChamps+1
	    ;new belts
	    If matchChamps>10 Then matchChamps=0
	    ;end
	   Until RewardApplicable(matchChamps)=1
      EndIf
	  If foc=11 
	 ...
	 If KeyDown(57)
	  If foc=11 Or foc=12   
	   ...
	   Repeat  
	    promoFoc=Rnd(1,200)
	   Until promoLib(promoFoc)=>0 And promoLocked(promoLib(promoFoc))=0
	  EndIf
	 EndIf
	 ;reverts
	 If KeyDown(14)
	...
	;new belts orig = 7
	If matchChamps<0 Then matchChamps=10
	If matchChamps>10 Then matchChamps=0
	;end
...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And keytim=0
	 ...
	 ;search left
	 If KeyDown(203) Or JoyXDir()=-1 Or MouseDown(2)
	  If foc=1 Then arenaPreset=arenaPreset-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=2 Then matchLocation=matchLocation-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=3 Then arenaAtmos=arenaAtmos-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=4 Then arenaAttendance=arenaAttendance-10 : PlaySound sMenuBrowse : keytim=5
	  If foc=5 Then arenaRopes=arenaRopes-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=6 Then arenaPads=arenaPads-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=7 Then arenaCanvas=arenaCanvas-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=8 Then arenaApron=arenaApron-1 : PlaySound sMenuBrowse : keytim=5
	  If foc=9 Then arenaMatting=arenaMatting-1 : PlaySound sMenuBrowse : keytim=5
	 EndIf
	 ;search right
	 If KeyDown(205) Or JoyXDir()=1 Or MouseDown(1)
	  If foc=1 Then arenaPreset=arenaPreset+1 : PlaySound sMenuBrowse : keytim=5
	 ...
	 If KeyDown(57)
	  PlaySound sMenuBrowse : keytim=5
	  If foc=1 Then arenaPreset=Rnd(1,no_arenas)
	  If foc=2 Then matchLocation=0
	  If foc=3 Then arenaAtmos=Rnd(0,no_atmos)
	  If foc=4 Then arenaAttendance=Rnd(0,10) : arenaAttendance=arenaAttendance*10
	  If foc=5 Then arenaRopes=Rnd(1,no_ropes)
	  If foc=6 Then arenaPads=Rnd(1,3)
	  If foc=7 Then arenaCanvas=Rnd(1,no_canvases)
	  If foc=8 Then arenaApron=Rnd(1,no_aprons)
	  If foc=9 Then arenaMatting=Rnd(0,3)
	 EndIf  
	 ;reverts
	 If KeyDown(14)
	  PlaySound sMenuBrowse : keytim=10
	  If foc=1 Then arenaPreset=1
	  If foc=2 Then matchLocation=0
	  If foc=3 Then arenaAtmos=0
	  If foc=4 Then arenaAttendance=0
	  If foc=5 Then arenaRopes=1
	  If foc=6 Then arenaPads=1
	  If foc=7 Then arenaCanvas=1
	  If foc=8 Then arenaApron=1
	  If foc=9 Then arenaMatting=0
	 EndIf 
	EndIf  
	;limits
	...
	EndIf
	If matchRopes=>2 Then arenaRopes=0
 ...
 DrawOption(4,rX#(400),rY#(y),"Attendance",arenaAttendance+"%") : y=378
...
Function DrawStatBox(rank,char,category,x,y)
 ...
 If category=2 Then stat=charStrength(char) : GetStatColour(charStrength(char),charOldStrength(char))
 If category=3 Then stat=charSkill(char) : GetStatColour(charSkill(char),charOldSkill(char))
 If category=4 Then stat=charAgility(char) : GetStatColour(charAgility(char),charOldAgility(char))
 If category=5 Then stat=charStamina(char) : GetStatColour(charStamina(char),charOldStamina(char))
 If category=6 Then stat=charToughness(char) : GetStatColour(charToughness(char),charOldToughness(char))
 ...
 If category=10 Then stat=CountMatches(char,0) : Color 255,255,255
 If category=11 Then stat=GetWinRate(char,0) : Color 255,255,255
 ;display text
 r=ColorRed() : g=ColorGreen() : b=ColorBlue()
 If category=<9 And b>200 Then r=255 : g=255 : b=130
 If foc<>rank Or subFoc=>20 Then r=r-(r/4) : g=g-(g/4) : b=b-(b/4)
 SetFont fontStat(1)
...
Function DrawProfile(char,x,y,display) ;-1=stats only, 0=normal, 1=left side only
 ...
 If display=>0
  ...
  If charManager(char)>0 And charPartner(char)>0 And charPartner(char)<>charManager(char)
   ...
   MaskImage charPhoto(editManager),24,4,0
   MaskImage charPhoto(editPartner),24,4,0
  EndIf
 ...
 If display=0
  x=rX#(400)+235
  ;manager portrait
  ;If charManager(char)>0 And charContract(char)>0 Then DrawImage charPhoto(charManager(char)),x-102,y+37
  ;bank account
  Color 200,150,50 : Rect x-50,y+5,100,5,1
  Color 0,0,0 : Rect x-50,y+5,100,5,0
  SetFont fontNews(2) : namer$="$UNDISCLOSED" : offset=0
  If game=1 And char=gamChar Then SetFont fontNews(4) : namer$="$"+GetFigure$(charBank(char)) : offset=-2
  If game=1 And char=gamChar And charBank(char)<0
   Outline(namer$,x,y+offset,0,0,0,200,50,50)
  Else
   Outline(namer$,x,y+offset,0,0,0,r,g,b)
  EndIf
  ;headers
  SetFont font(1)
  Outline("Experience:",x-59,y+21,0,0,0,r,g,b)
  Outline("Contract:",x-52,y+36,0,0,0,r,g,b)
  Outline("Salary:",x-46,y+51,0,0,0,r,g,b)
  ;stats
  SetFont fontStat(1)
  OutlineStraight(CountMatches(char,0)+" matches",x-20,y+20,0,0,0,255,255,255)
  offset=3+StringWidth(CountMatches(char,0)+" matches")
  SetFont fontStat(0)
  OutlineStraight("("+GetWinRate(char,0)+"% wins)",(x-20)+offset,y+20,0,0,0,255,255,255)
  SetFont fontStat(1)
  ...
  If charSalary(char)>0 Then namer$="$"+GetFigure$(charSalary(char))+" per week" Else namer$="None" 
  If char=fedBooker(charFed(char)) Then namer$="Undisclosed"
  OutlineStraight(namer$,x-20,y+50,0,0,0,255,255,255)
  ;smallprint headers
  SetFont font(0)
...
If TitleHolder(char,3) And TitleHolder(char,5)=0 Then DrawImage gBelt(4),x,y
  If TitleHolder(char,3) And TitleHolder(char,5) Then DrawImage gBelt(4),x,y-3
  If TitleHolder(char,5) Then DrawImage gBelt(4),x,y

If TitleHolder(char,3) And TitleHolder(char,6)=0 Then DrawImage gBelt(5),x,y
  If TitleHolder(char,3) And TitleHolder(char,6) Then DrawImage gBelt(5),x,y-3
  If TitleHolder(char,6) Then DrawImage gBelt(5),x,y

If TitleHolder(char,3) And TitleHolder(char,7)=0 Then DrawImage gBelt(6),x,y
  If TitleHolder(char,3) And TitleHolder(char,7) Then DrawImage gBelt(6),x,y-3
  If TitleHolder(char,7) Then DrawImage gBelt(6),x,y
;end  
 EndIf
End Function
...
Function DrawFedProfile(cyc,x,y)
 ...
 ;portraits
 If fedChampWorld(cyc)>0 
  DrawImage charPhoto(fedChampWorld(cyc)),x-127,y+23
  MaskImage charPhoto(fedChampWorld(cyc)),24,4,0
  DisplayBelts(fedChampWorld(cyc),x-127,(y+31)-(charHeight(fedChampWorld(cyc))/2))
 EndIf
 If fedBooker(cyc)>0 Then DrawImage charPhoto(fedBooker(cyc)),x-102,y+24
 MaskImage charPhoto(fedBooker(cyc)),24,4,0
 ;headers
 SetFont font(1)
 ...
 Outline("100",x+46,y+21,0,0,0,r2,g2,b2) 
 ;reputation data
 ...
 OutlineStraight("$"+GetFigure$(CountSalaries(cyc))+" per week",x-18,y+51,0,0,0,255,255,255)
 OutlineStraight("Undisclosed",x-18,y+66,0,0,0,255,255,255)
End Function
...
Function RewardApplicable(reward)
 ...
 If reward>0 And slot>0 Then applicable=0 ;no title changes in career universes
 Return applicable
End Function
...
Function MatchLoader(scriptA$,scriptB$)
 Cls
  ...
  If no_wrestlers=3 And matchTeams=<0
   ...
   MaskImage charPhoto(pChar(3)),24,4,0
  EndIf
  If no_wrestlers=4 And matchTeams=<0
   ...
   MaskImage charPhoto(pChar(4)),24,4,0
  EndIf
  ;team display
  If matchTeams>0
   ...
   For cyc=1 To no_wrestlers
    ...
    MaskImage charPhoto(pChar(cyc)),24,4,0
   Next
  EndIf
  ;big match display
  If no_wrestlers>4 And matchTeams=<0
...
Function ChangeResolution(resolution,task) ;0=pre-game, 1=during game
 ...
 If GfxMode3DExists(width,height,16)=2 Then width=1440 : height=856 : optGameRes=2 : optMenuRes=2
 ;make transition?
 If width<>GraphicsWidth() Or height<>GraphicsHeight()
  If task>0 Then Loader("Please Wait","Adjusting Resolution")
  Graphics3D width,height,16,2+optWindow
;Graphics3D 1440,856,16,2  
...
Function ManageMusic(track)
 ...
 If (screen=>50 And screen<>51) Or screen=6 Then chThemeTarget#=chThemeTarget#/2 : speeder#=0.01
 If screen=54 And trainStage=2 Then chThemeTarget#=0.0
