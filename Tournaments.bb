	 ...
	 If KeyDown(203) Or JoyXDir()=-1 Or MouseDown(2)
	  ...
	  If foc=3 
	   Repeat
	    ...
	    If optCupReward>0 And (optCupFed=0 Or slot>0) Then satisfied=0
...
;//////////////////////////////////////////////////////////////////////////////
Function Tournament()
;update remaining brackets
If game=1 And cupSpan(cupSlot)=1
 starBracket=TournamentStatus(gamChar)
 If starBracket>0
  For b=1 To 31
   If cupBracket(cupSlot,b,1)>0 And cupBracket(cupSlot,b,1)<>gamChar And cupBracket(cupSlot,b,2)>0 And cupBracket(cupSlot,b,2)<>gamChar And cupResult(cupSlot,b)=0 
    If CupStage(b)=>CupStage(starBracket) Then SimulateBracket(b)
   EndIf 
  Next
 Else
  Repeat
   For b=1 To 31
    If cupBracket(cupSlot,b,1)>0 And cupBracket(cupSlot,b,2)>0 And cupResult(cupSlot,b)=0 Then SimulateBracket(b)
   Next
  Until cupResult(cupSlot,1)>0
 EndIf
 If cupResult(cupSlot,1)>0 Then PlaySound sCrowd(9)
EndIf
;find next match
For b=1 To 31
 If cupBracket(cupSlot,b,1)>0 And cupBracket(cupSlot,b,2)>0 And cupResult(cupSlot,b)=0
  If cupBracket(cupSlot,b,1)=gamChar Or cupBracket(cupSlot,b,2)=gamChar Or game=0 Then cupFoc(cupSlot)=b
 EndIf
Next
foc=cupFoc(cupSlot)
...
While go=0
 ...
 For framer=1 To frames
	...
	If gotim>20 And scrollReveal=1 And keytim=0
	 ...
	 If KeyDown(28) Or ButtonPressed() Or MouseDown(1)
	  ;play/simulate match
	  If foc=>1 And foc=<31 And (game=0 Or TournamentStatus(gamChar)=0)
	  ...
	  ;leave/proceed 
	  If foc=0 
	   If game=1 And cupFoc(cupSlot)>0 And TournamentStatus(gamChar)>0 Then go=1 Else go=-1
	  EndIf
	 EndIf
	EndIf
	;force players
 ...
 For b=1 To 32
  If b<cupSize(cupSlot)
   ...
   If game=0 And cupBracket(cupSlot,b,1)>0 And cupBracket(cupSlot,b,2)>0 And cupResult(cupSlot,b)=0 And scrollReveal=1
 ...
 If scrollReveal=1
  If game=0 
   DrawOption(32,rX#(400)+scroll,rY#(515),"REFRESH CHARACTERS","")
   DrawOption(0,rX#(400)+scroll,rY#(560),"<<< SAVE & EXIT <<<","")  
  EndIf
  If game=1
   ;If TournamentStatus(gamChar)>0 Then namer$=">>> PROCEED >>>" Else namer$="<<< SAVE & EXIT <<<"
   namer$=">>> PROCEED >>>"
   DrawOption(0,rX#(400)+scroll,rY#(560),namer$,"")
  EndIf  
 EndIf
 ;diagnostics
 ;SetFont fontStat(1) 
 ;Outline("game: "+game,rX#(100),50,0,0,0,255,255,255)
 ;cursor
 If scrollReveal=1
  If foc<>oldfoc Then oldfoc=foc : PlaySound sMenuSelect  
  DrawImage gCursor,MouseX(),MouseY()
...
If go=-1
 ...
 screen=1
 If game=1 And cupSize(cupSlot)>0 Then screen=20
 If game=1 And cupSize(cupSlot)=0 Then screen=24
EndIf
;construct match
If go=1
 fed=cupFed(cupSlot)
...
Function GenerateTournament(promotion,teams,reward,size,selection,date,span)
 ...
 If selection>0
  ...
  For cyc=1 To cupSize(cupSlot)
   ...
   Repeat
    ...
    Next
    If game=1 And newbie=gamChar Then conflict=1
    If game=1 And teams>0 And newbie=charPartner(gamChar) Then conflict=1
 ...
 If selection>0 And game=0
  ...
  EndIf
 EndIf
 ;insert star character
 If game=1
  cupChar(cupSlot,Rnd(1,cupSize(cupSlot)))=gamChar
  cupCharControl(cupSlot,gamChar)=3
 EndIf
 ;assign partners
...
Function DrawBracket(bracket,cyc,x,y)
 ...
 If char>0
  If foc=bracket Or (game=1 And cupFoc(cupSlot)=bracket) Then DrawImage gMenu(5),x,y+1 Else DrawImage gMenu(6),x,y+1
  ...
  r=200 : g=200 : b=200
  If foc=bracket Or (game=1 And cupFoc(cupSlot)=bracket) Then r=255 : g=255 : b=255
 ...
 Else
  If foc=bracket Or (game=1 And cupFoc(cupSlot)=bracket) Then shower=5 Else shower=6
