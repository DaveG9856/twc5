...
Function PrepareCast()
 ...
 If no_refs>0
  For count=1 To no_refs
   ...
   EndIf
   pRole(cyc)=1
   If game=1 And pChar(cyc)=gamChar Then pControl(cyc)=3 
  Next
 EndIf
 ;add managers
 If (optManagers=1 And no_wrestlers<4) Or (optManagers=2 And no_wrestlers=<4) Or (optManagers=3 And no_wrestlers=<8)
  For cyc=1 To no_wrestlers
   ...
   If game=1 And screenAgenda=14 And pChar(cyc)=negChar Then manager=gamChar
   If manager>0 And FindCharacter(manager)=0 And no_plays<optPlayLim
    no_plays=no_plays+1
...
Function AddIntruder()
 ...
 Repeat
  ...
  For v=1 To no_wrestlers
   If charRelationship(newbie,pChar(v))<>0 Then purpose=1
  Next
  randy=Rnd(0,1)
  ...
  ;urgent exceptions
  If FindCharacter(newbie)>0 Or charFed(newbie)=9 Then satisfied=0
...
Function LoadPlayers()
 For cyc=1 To no_plays
  ...
  pDT(cyc)=0 : pHeat(cyc)=50 : pAbused(cyc)=0
  pDizzyTim(cyc)=0 : pBlindTim(cyc)=0 : pImmunity(cyc)=0
...
Function FindElevation(cyc)
 ...
 If v>0 And pAnim(cyc)<>160 And pAnim(cyc)<>161 And pAnim(cyc)<300
  If covering=0 Or pAnim(v)<>150 Or AttackViable(cyc)=<2
   If pPinCount(v)=0 Then Pop(v,Rnd(2,8),0.5)
...
Function MonitorStatus(cyc) 
 ...
 If pHealthLimit(cyc)>pHealth(cyc)+(1250*optLength) Then pHealthLimit(cyc)=pHealthLimit(cyc)-1
...
Function ManageEntrance(cyc)
 If matchEnter=cyc And pRole(cyc)=0 And matchState=>1 And matchTim>50
  ...
  If pOutTim(cyc)=0
   ...
   If fed<>7 Or arenaPreset=>11
    PlayTheme(pChar(cyc))
    If game=1 And musicLearned(charTheme(pChar(cyc)))<1 Then musicLearned(charTheme(pChar(cyc)))=1
   EndIf
   For v=1 To no_plays
  ...
  If pOutTim(cyc)=>125 And ChannelPlaying(chTheme) And matchState<>4
   ...
   If chThemeVol#=<0.3 Or pOutTim(cyc)>225 Then arenaLight=1
...
;new belts
If TitleHolder(pChar(cyc),5) Then comSuffix$="Womens Champion"
If TitleHolder(pChar(cyc),6) Then comSuffix$="US Champion"
If TitleHolder(pChar(cyc),7) Then comSuffix$="TV Champion"
;end
