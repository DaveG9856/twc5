...
Function AttackAnims(cyc)
 ...
 If pAnim(cyc)=>60 And pAnim(cyc)=<63
  ...
  If attackFall(style,move)>0
   ...
   If pAnimTim(cyc)=Int(attackFall(style,move)/pAnimSpeed#(cyc))
    ...
    If pSting(cyc)=1 Then pHealth(cyc)=pHealth(cyc)-100 : pHP(cyc)=pHP(cyc)-100 : pHurtA#(cyc)=Rnd(0,360)
 ...
 If pAnim(cyc)=64
  ...
  EndIf
  ;flight physics
  If pAnimTim(cyc)=Int(attackTransition(style,move)/pAnimSpeed#(cyc)) Then ProduceSound(p(cyc),sSwing,20000,1) : Pop(0,Rnd(2,9),1)
 ...
 If pAnim(cyc)=69
  ...
  If pAnimTim(cyc)=>Int((transition+5)/pAnimSpeed#(cyc)) And pAnimTim(cyc)=<Int((transition+25)/pAnimSpeed#(cyc)) And pSting(cyc)=1 And pScar(cyc,43)=<4
   FindAttackImpacts(cyc,transition+5,transition+15,transition+25,20,15,200,0,2)
  EndIf
...
Function CrushAnims(cyc)
 ...
 If pAnim(cyc)=74
  ...
  EndIf
  ;travelling
  If pAnimTim(cyc)=Int(crushTransition(style,move)/pAnimSpeed#(cyc)) Then ProduceSound(p(cyc),sSwing,20000,1) : Pop(0,Rnd(2,9),1)
...
Function FindAttackImpacts(cyc,activeTim,impactTim,expireTim,baseRange#,height#,basePower,weapon,style)
 ...
 For v=1 To no_plays
  ...
  If cyc<>v And contact#<range#+blockOffset# And pY#(cyc)+TranslateHeight#(cyc,height#)>pY#(v) And pY#(cyc)+TranslateHeight#(cyc,height#)<pY#(v)+35 And pSting(cyc)=1
   impact=0
   If (InLine(cyc,pLimb(v,36),angle#) And pAnim(cyc)<>69) Or (InRearView(cyc,pLimb(v,36),angle#) And pAnim(cyc)=69)
   ...
   If impact>0
    ...
    If blocked=0 
     ...
     If pSpecial(cyc)>0 And style=>3 Then pHP(v)=0
...
Function GroundImpactChecks(cyc)
 ...
 If pPinning(cyc)>0 And pAnim(cyc)<300 And pAnim(pPinning(cyc))<300
  ...
  If pPinCount(v)=>2 Then Pop(v,Rnd(2,9),1) : Pop(cyc,Rnd(2,9),0)
  GroundReaction(v)
 ...
 If pPinner(cyc)>0 And pAnim(cyc)<300 And pAnim(pPinner(cyc))<300
  v=pPinner(cyc)
  If pPinCount(cyc)=1 Then Pop(cyc,Rnd(2,9),0)
  If pPinCount(cyc)=>2 Then Pop(cyc,Rnd(2,9),1) : Pop(cyc,Rnd(2,9),0)
...
Function ClockIntruder(cyc,v)
 If game=1 And matchState=3 And matchPlayer>0 And pRole(v)=0 And pEliminated(v)=0
  If pRole(cyc)=3 Or (pRole(cyc)=2 And pClient(cyc)=0) Or (pRole(cyc)=2 And v=matchPlayer And pClient(cyc)=matchPlayer)
   If pTeam(v)=pTeam(matchPlayer) Then matchIntruder=cyc ;hinder player
   If pTeam(v)<>pTeam(matchPlayer) Then matchIntruder=-cyc ;benefit player
   If charRelationship(pChar(cyc),gamChar)>0 Or pChar(cyc)=charPartner(gamChar) Or pChar(cyc)=charManager(gamChar) 
    If v=matchPlayer Then matchBetrayal=cyc ;clock betrayal
   EndIf
  EndIf
 EndIf
End Function
...
Function ClockAbuse(cyc,v,level)
 If game=1 And matchTeams>0 And cyc=matchPlayer And pTeam(v)=pTeam(cyc)
  pAbused(v)=pAbused(v)+level
 EndIf
End Function