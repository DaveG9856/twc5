...
Function SaveUniverse()
 file=WriteFile("Data/Universe.dat")
  ...
  For cyc=1 To 10
   ...
   WriteInt file,hiTitles(cyc)
   WriteInt file,hiSuccess(cyc)
   WriteInt file,hiTalent(cyc)
...
Function LoadUniverse()
 file=ReadFile("Data/Universe.dat")
  ...
  For cyc=1 To 10
   ...
   hiTitles(cyc)=ReadInt(file)
   hiSuccess(cyc)=ReadInt(file)
   hiTalent(cyc)=ReadInt(file)
...
Function SaveProgress(directory)
 ...
 If directory=>0 Then file=WriteFile("Data/Slot0"+directory+"/Progress.dat")
  ...
  WriteInt file,gamCasted
  For count=1 To 20
   WriteInt file,gamCastList(count)
  Next
  WriteInt file,gamGimmickChanges
  ;schedule
  ...
  For count=1 To 48
   WriteInt file,gamSchedule(count)
   WriteInt file,gamRivalFed(count)
   WriteInt file,gamOpponent(count) 
   WriteInt file,gamMatch(count) 
   WriteInt file,gamGimmick(count) 
   WriteInt file,gamPromo(count) 
   WriteInt file,gamPromoVariable(count) 
   WriteInt file,gamResult(count)
   ;arena properties
   WriteInt file,gamVenue(count) 
   WriteInt file,gamAtmos(count)
   WriteInt file,gamAttendance(count)
   WriteInt file,gamRopes(count)
   WriteInt file,gamPads(count)
   WriteInt file,gamCanvas(count)
   WriteInt file,gamApron(count)
   WriteInt file,gamMatting(count)
  Next
  ;agreements
  For count=1 To 50
  ...
  For style=1 To 4
   For move=1 To 200
    WriteInt file,moveLearned(style,move)
   Next
  Next
  For style=1 To 5
   For move=1 To 50
    WriteInt file,attackLearned(style,move)
    WriteInt file,crushLearned(style,move)
   Next
  Next
  For move=1 To 200
   WriteInt file,tauntLearned(move)
  Next
  For coz=1 To 3
   For count=1 To 200
    WriteInt file,costumeLearned(coz,count)
   Next
  Next
  For count=1 To 100
   WriteInt file,musicLearned(count)
  Next
  ;promo locks
  For count=1 To 200
   WriteInt file,promoLocked(count)
  Next
  ;negotiation history
...
Function LoadProgress(directory)
 ...
 If directory=>0 Then file=ReadFile("Data/Slot0"+directory+"/Progress.dat")
  ...
  gamCasted=ReadInt(file)
  For count=1 To 20
   gamCastList(count)=ReadInt(file)
  Next
  gamGimmickChanges=ReadInt(file)
  ;schedule
  ...
  For count=1 To 48
   gamSchedule(count)=ReadInt(file)
   gamRivalFed(count)=ReadInt(file)
   gamOpponent(count)=ReadInt(file)
   gamMatch(count)=ReadInt(file) 
   gamGimmick(count)=ReadInt(file) 
   gamPromo(count)=ReadInt(file) 
   gamPromoVariable(count)=ReadInt(file) 
   gamResult(count)=ReadInt(file)
   ;arena properties
   gamVenue(count)=ReadInt(file) 
   gamAtmos(count)=ReadInt(file)
   gamAttendance(count)=ReadInt(file)
   gamRopes(count)=ReadInt(file)
   gamPads(count)=ReadInt(file)
   gamCanvas(count)=ReadInt(file)
   gamApron(count)=ReadInt(file)
   gamMatting(count)=ReadInt(file)
  Next
  ;editor blocks
  For count=1 To 50
  ...
  For style=1 To 4
   For move=1 To 200
    moveLearned(style,move)=ReadInt(file)
   Next
  Next
  For style=1 To 5
   For move=1 To 50
    attackLearned(style,move)=ReadInt(file)
    crushLearned(style,move)=ReadInt(file)
   Next
  Next
  For move=1 To 200
   tauntLearned(move)=ReadInt(file)
  Next
  For coz=1 To 3
   For count=1 To 200
    costumeLearned(coz,count)=ReadInt(file)
   Next
  Next
  For count=1 To 100
   musicLearned(count)=ReadInt(file)
  Next
  ;promo locks
  For count=1 To 200
   promoLocked(count)=ReadInt(file)
  Next
  ;negotiation history
  For v=1 To optCharLim
   For count=1 To 3
...
;new belts
WriteInt file,fedChampWomens(cyc)
WriteInt file,fedChampUS(cyc)
WriteInt file,fedChampTV(cyc)
;end
   For count=1 To 2
...
;new belts
fedChampWomens(cyc)=ReadInt(file)
fedChampUS(cyc)=ReadInt(file)
fedChampTV(cyc)=ReadInt(file)
;end
   For count=1 To 2
...
Function SaveChar(char,directory)
  ...
  If directory=>0 Then file=WriteFile("Data/Slot0"+directory+"/Character"+Dig$(char,100)+".dat")
   ...
   WriteInt file,charEyeShape(char);eye fix?
...
Function LoadPhotos(directory)
 For char=0 To no_chars
  ...
  charPhoto(char)=LoadImage(filer$+"Photo000.bmp")
  If charPhoto(char)=0
  ...
  EndIf
  If char=0 Then charPhotoR(char)=24 : charPhotoG(char)=4 : charPhotoB(char)=0
...
Function CheckLimits(char)
 ...
 If charEyes(char)<1 Then charEyes(char)=5
 If charEyes(char)>5 Then charEyes(char)=1
 ...
 If charHeadSize(char)<70 Then charHeadSize(char)=70
 If charHeadSize(char)>125 Then charHeadSize(char)=125  
...
Function MoveChar(char,target)
 If charFed(char)<>target And fedSize(target)<optRosterLim
  ...
  If (game=1 Or slot>0) And char=gamChar
