...
Function LoadArena()
 ...
 If screen>0
  ...
  Else
   ...
   If fed>0 Then tVideo(4)=LoadTexture("World/Banners/Poster"+Dig$(Rnd(1,18),10)+".JPG")
   tVideo(5)=LoadTexture("World/Banners/Poster"+Dig$(Rnd(1,18),10)+".JPG")
  EndIf
 ...
 For count=1 To 14
  If FindChild(world,"Banner"+Dig$(count,10))>0
   If screen>0 And (optDetail=<1 Or screenAgenda=10 Or (fed=7 And arenaAttendance=0))
 ...
 If screen>0 And optDetail=<1
  For count=1 To 4
   If FindChild(world,"Trash"+Dig$(count,10))>0
    HideEntity FindChild(world,"Trash"+Dig$(count,10))
   EndIf
  Next
  For count=1 To 13
   If FindChild(world,"WallPic"+Dig$(count,10))>0
    HideEntity FindChild(world,"WallPic"+Dig$(count,10))
   EndIf
  Next
  For count=1 To 16
   If FindChild(world,"Light"+Dig$(count,10))>0
    HideEntity FindChild(world,"Light"+Dig$(count,10))
   EndIf
  Next
 EndIf
 ;reduced detail
 If screen>0 And optDetail=0
  For count=1 To 4
   If FindChild(world,"Steps"+Dig$(count,10))>0
    HideEntity FindChild(world,"Steps"+Dig$(count,10))
   EndIf
  Next
  For count=1 To 11
   If FindChild(world,"Locker"+Dig$(count,10))>0
    HideEntity FindChild(world,"Locker"+Dig$(count,10))
   EndIf
  Next
  HideEntity FindChild(world,"Plant01")
  HideEntity FindChild(world,"Plant02")
  HideEntity FindChild(world,"Plant03")
  HideEntity FindChild(world,"Plant04")
  HideEntity FindChild(world,"Plant05")
  HideEntity FindChild(world,"Bulletin B")
  HideEntity FindChild(world,"Bulletin B01")
 EndIf
 ;crowd
 crowdLimit=0
 If arenaType=1
 ...
 For count=1 To 300
  ...
  EndIf
  If FindChild(world,"TN3DC"+Dig$(count,100))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"TN3DC"+Dig$(count,100))
  EndIf
  If FindChild(world,"TN3DS"+Dig$(count,100))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"TN3DS"+Dig$(count,100))
  EndIf
  If FindChild(world,"Security"+count)>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"Security"+count)
  EndIf
  If FindChild(world,"Security0"+count)>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"Security0"+count)
  EndIf
  If FindChild(world,"CManStd"+Dig$(count,10))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"CManStd"+Dig$(count,10))
  EndIf
  If FindChild(world,"CManKne"+Dig$(count,10))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"CManKne"+Dig$(count,10))
  EndIf
  If FindChild(world,"CMKCam"+Dig$(count,10))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"CMKCam"+Dig$(count,10))
  EndIf
  If FindChild(world,"CMSCam"+Dig$(count,10))>0
   If arenaAttendance=0 Then HideEntity FindChild(world,"CMSCam"+Dig$(count,10))
  EndIf
  If FindChild(world,"Fern"+Dig$(count,10))>0
   If arenaAttendance>0 Then HideEntity FindChild(world,"Fern"+Dig$(count,10))
  EndIf
  If FindChild(world,"Railing"+Dig$(count,10))>0
   EntityShininess FindChild(world,"Railing"+Dig$(count,10)),.75
  EndIf
 Next 
 ;load scenery blocks
...
Function GenerateAttendance()
 ...
 For cyc=1 To no_wrestlers
  popper=popper+charPopularity(pChar(cyc))
  If TitleHolder(pChar(cyc),1) Then popper=popper+charPopularity(pChar(cyc))
  If TitleHolder(pChar(cyc),2) Then popper=popper+(charPopularity(pChar(cyc))/2)
  If TitleHolder(pChar(cyc),3) Then popper=popper+(charPopularity(pChar(cyc))/4)
 Next
 If no_wrestlers>0 Then popper=popper/no_wrestlers
 ;promotion popularity
 fedPopper=fedPopularity(fed)
 If fed=0 Then fedPopper=100 
 If fed=7 Then fedPopper=50 
 ;calculate core attendance
 value=(popper-30)*(fedPopper-30)
 value=Rnd(value,value*2)
 If value>5000 Then value=5000
 ;magnify for big venues
 If arenaPreset=>11
  magnifier#=(Float#(popper)/20)+(Float#(fedPopper)/20)
  value=Int(value*magnifier#)
  If gamSchedule(gamDate)<2 Then value=value/2
 EndIf
 ;negate for school
 If fed=7 And gamSchedule(gamDate)<2 Then value=0
...
Function TranslateAttendance(attendance)
 value=Int(GetPercent#(attendance,5000))
 If arenaPreset=>11 Then value=Int(GetPercent#(attendance,50000))
 Return value
End Function
...
Function DecorateRing()
 ...
 ;matting
 If arenaMatting>0
  tMatting=LoadTexture("World/Mats/Mat02.JPG")
...
Function PrepareScenery()
 ...
 If screen=53 And negTopic=65 Then blackout=1
 ;plants
 For count=1 To 20
...
Function LoadBlocks()
 ...
 ;37-38 = FREE
 ;toilet cubicles
 n=39 : blockType(n)=2 : blockClimb(n)=0
...
Function LoadAtmos()
 ...
 If screen>0
  If arenaPreset=<24 Then sAtmos=LoadSound("Sound/Crowd/Atmosphere.ogg")
  If arenaPreset=25 Then sAtmos=LoadSound("Sound/Crowd/City.ogg") 
  If arenaPreset=26 Then sAtmos=LoadSound("Sound/Crowd/Beach.ogg") 
  LoopSound sAtmos
...
Function ManageAtmos()
 ...
 If screen=0 Then changer#=PercentOf#(10.0,gotim/2)
 ...
 Next
 For count=1 To 30
  If FindChild(world,"Bulb"+Dig$(count,10))>0
...
Function Camera()
 ...
 If camType=10 And camFoc>0
  ...
  EndIf
  If screen=55
   If camFoc=1 Or camFoc=4 Then swing#=-15
   If camFoc=2 Or camFoc=5 Then swing#=15
  EndIf
 ...
 If camType=11
  ...
  If randy=<1 Or gotim=0 Or camType<>camOldType Or camFoc<>camOldFoc Or matchState<>matchOldState
  ...
  While camTX#>camPivTX#-range# And camTX#<camPivTX#+range# And camTZ#>camPivTZ#-range# And camTZ#<camPivTZ#+range# And camTY#<camPivTY#+60
   ...
   randy=Rnd(0,1) 
   If randy=0 Then camTY#=camPivTY#+Rnd(-20,60) Else camTY#=camPivTY#+Rnd(-20,100) 
 ...
 EndIf
 ;intro override
 If matchState=0 And matchLocation=0 And screenAgenda<>10
 ...
 EndIf
 ;entrance protection
 If matchState=1 And matchLocation=0 And matchEnter>0 And pOutTim(matchEnter)=0
 ...
 If screen=54
  ...
  EndIf
 EndIf
 ;press conference override
 If screen=55 And negStage=5 And negTim>1000
  camTX#=-110 : camTY#=90 : camTZ#=-150
 EndIf
 ;ADJUST TARGETS
 heightAssist=0
 ...
 EndIf
 ;intro arena scan
 If matchState=0 And matchEntrances>0
 ...
 If gotim>0 
  ...
  ;If screen=55 And negStage=5 And negTim>1000 Then speeder=480
