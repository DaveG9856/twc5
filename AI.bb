...
Function TranslateInput(cyc)
 ...
 EndIf
 ;corruption declaw
 ;If game=1 And gamAgreement(11)>0 And pChar(cyc)=gamOpponent(gamDate) And pFoc(cyc)=matchPlayer Then cAttack(cyc)=0 : cGrapple(cyc)=0
 If game=1 And gamAgreement(12)>0 And cyc=matchPlayer And pChar(pFoc(cyc))=gamOpponent(gamDate) Then cAttack(cyc)=0 : cGrapple(cyc)=0
 ;forced entrance
...
Function GetIntensity(cyc)
 ...
 ;minimum performance for key matches
 If game=1 And pControl(cyc)=0 And matchReward=>2 And matchReward=<5
 ...
 ;lying down
 If game=1 And gamAgreement(11)>0 And pChar(cyc)=gamOpponent(gamDate) And pFoc(cyc)=matchPlayer Then value=10000
 Return value
End Function
...
Function LegalMan(cyc)
 ...
 If pRole(cyc)>0 Or (matchState=>3 And pEliminated(cyc)) Then value=0
 Return value
End Function
